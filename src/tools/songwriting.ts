...
export async function registerSongwritingTools(server: McpServer) {
  ...
  const generateLyricsContract = songwritingContracts.generateLyrics;

  server.registerTool(
    generateLyricsContract.name,
    {
      title: generateLyricsContract.title,
      description: generateLyricsContract.description,
      inputSchema: generateLyricsInputSchema.shape,
      outputSchema: generateLyricsOutputSchema.shape
    },

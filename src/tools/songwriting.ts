...
export async function registerSongwritingTools(server: McpServer) {
  ...
  server.registerTool(
    'generate_lyrics',
    {
      ...
      inputSchema: GenerateLyricsRequestSchema.shape,
      outputSchema: GenerateLyricsResponseSchema.shape
    },
    async ({ concept, style, tone, length, constraints, reference_style }: GenerateLyricsRequest) => {

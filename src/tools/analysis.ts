...
export async function registerAnalysisTools(server: McpServer) {
  ...
  server.registerTool(
    'emotional_archaeology',
    {
      ...
      inputSchema: EmotionalArchaeologyRequestSchema.shape,
      outputSchema: EmotionalArchaeologyResponseSchema.shape
    },
    async ({ data_sources, time_period, emotional_depth, privacy_level, theme_focus }: EmotionalArchaeologyRequest) => {

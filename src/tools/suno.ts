...
export async function registerSunoTools(server: McpServer) {
  ...
  server.registerTool(
    'format_for_suno',
    {
      ...
      inputSchema: FormatForSunoRequestSchema.shape,
      outputSchema: FormatForSunoResponseSchema.shape
    },
    async ({ lyrics, style_tags, structure_explicit, optimize_for }: FormatForSunoRequest) => {
